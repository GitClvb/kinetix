...
import com.kinetix.fit.service.ColorService;
...
public class ColorController {
    ...
    @GetMapping("/obtener")
    public ResponseEntity<List<ColorDTO>>
    obtenerColores() {
        return ResponseEntity.ok(
                colorService.obtenerColores()
        );
    }

    @PostMapping("/catalogo")
    public ResponseEntity<?> crearColor(
            @RequestBody CrearColorCatalogoDTO dto
    ) {
        ...
        if (colorRepository.existsByNombre(dto.getNombre())) {
            ...
            return ResponseEntity
                    ...
                    .body("El color ya existe");

        }

        return ResponseEntity.ok(
                colorService.crearColor(dto)
        );
    }
}